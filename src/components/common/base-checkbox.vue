<template>
  <div class="rounded-lg" :class="class">
    <input :id="id" :name="name"
      ...
      type="checkbox" :checked="value" :disabled="disabled" @click="onClick" @focusout="onFocusout" @keyup="onKeyup"
      @change="$emit('input', $event.target.checked); $emit('change', $event.target.checked)">
    <label class="w-full py-3 ml-2 text-sm font-medium text-gray-900 dark:text-gray-300" :for="name">
      <slot>{{ label }}</slot>
    </label>
  </div>
</template>
